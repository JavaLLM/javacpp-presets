...
CMAKE_COMMON_OPTS="-DBUILD_SHARED_LIBS=ON -DLLAMA_BUILD_EXAMPLES=OFF -DLLAMA_BUILD_TESTS=OFF"
...
case $PLATFORM in
  ...
  linux-*)
    if [[ "$EXTENSION" == *cuda ]]; then
      cmake .. $CMAKE_COMMON_OPTS -DLLAMA_AVX=ON -DLLAMA_AVX2=ON -DLLAMA_CUBLAS=1
    else
      cmake .. $CMAKE_COMMON_OPTS -DLLAMA_AVX=ON -DLLAMA_AVX2=ON
    fi
    ;;
